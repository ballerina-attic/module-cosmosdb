...
public type Client client object {
    ...
    private http:Client documentClient;
...
public remote function Client.listDatabases(RequestOptions? requestOptions = ())
                           returns DatabaseListResponse|error {
    string requestPath = string `/{{DB_PATH}}`;
        ...
        if (httpResponse is http:Response) {
            ...
            if (jsonResponse is json) {
                if (statusCode == http:OK_200) {
                    ...
                    return setResponseError(jsonResponse.message.toString());
                }
            } else {
                return setResponseError(JSON_EXTRACTION_ERROR_MSG);
            }
        } else {
...
public remote function Client.createDatabase(string databaseId, RequestOptions? requestOptions = ())
                           returns DatabaseResponse|error {
    string requestPath = string `/{{DB_PATH}}`;
    http:Request request = new;
...
public remote function Client.getDatabase(string databaseId, RequestOptions? requestOptions = ())
                           returns DatabaseResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}`;
    string requestPath = string `/{{resourceLink}}`;
    http:Request request = new;
...
public remote function Client.deleteDatabase(string databaseId, RequestOptions? requestOptions = ())
                           returns ResourceResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}`;
    string requestPath = string `/{{resourceLink}}`;
    http:Request request = new;
...
public remote function Client.createCollection(string databaseId, string collectionId,
                           CollectionCreateOptions? collectionCreateOptions = ()) returns CollectionResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}`;
    string requestPath = string `/{{resourceLink}}/{{COLL_PATH}}`;
    http:Request request = new;
...
public remote function Client.listCollections(string databaseId, RequestOptions? requestOptions = ())
                           returns DocumentCollectionsResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}`;
    string requestPath = string `/{{resourceLink}}/{{COLL_PATH}}`;
    http:Request request = new;
...
public remote function Client.getCollection(string databaseId, string collectionId, RequestOptions? requestOptions = ())
                           returns CollectionResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}/{{COLL_PATH}}/{{collectionId}}`;
    string requestPath = string `/{{resourceLink}}`;
    http:Request request = new;
    var requestHeaders = costructRequestHeaders(request, "get", COLL_PATH, resourceLink, self.masterKey,
        JSON_CONTENT_TYPE, requestOptions = requestOptions);
...
public remote function Client.deleteCollection(string databaseId, string collectionId, RequestOptions? requestOptions = ())
                           returns ResourceResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}/{{COLL_PATH}}/{{collectionId}}`;
    string requestPath = string `/{{resourceLink}}`;
    http:Request request = new;
...
public remote function Client.createDocument(string databaseId, string collectionId, string documentId,
                                      DocumentCreateOptions? documentCreateOptions = (), json content)
                           returns ResourceResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}/{{COLL_PATH}}/{{collectionId}}`;
    string requestPath = string `/{{resourceLink}}/{{DOC_PATH}}`;
    http:Request request = new;
...
public remote function Client.listDocuments(string databaseId, string collectionId,
                                     DocumentListOptions? documentListOptions = ()) returns DocumentListResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}/{{COLL_PATH}}/{{collectionId}}`;
    string requestPath = string `/{{resourceLink}}/{{DOC_PATH}}`;
    http:Request request = new;
...
public remote function Client.queryDocuments(string databaseId, string collectionId, @sensitive string queryString,
                                      DocumentQueryOptions? documentQueryOptions = (), any... parameters)
                           returns DocumentListResponse|error {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}/{{COLL_PATH}}/{{collectionId}}`;
    string requestPath = string `/{{resourceLink}}/{{DOC_PATH}}`;
...
public remote function Client.getDocument(string databaseId, string collectionId, string documentId,
                                   RequestOptions? requestOptions = ()) returns DocumentResponse?|error? {
    string resourceLink = string `{{DB_PATH}}/{{databaseId}}/{{COLL_PATH}}/{{collectionId}}/{{DOC_PATH}}/{{documentId}}`;
    string requestPath = string `/{{resourceLink}}`;
    http:Request request = new;
    var requestHeaders = costructRequestHeaders(request, "get", DOC_PATH, resourceLink, self.masterKey,
        JSON_CONTENT_TYPE, requestOptions = requestOptions);
...
public remote function Client.deleteDocument(string databaseId, string collectionId, string documentId,
                                      RequestOptions? requestOptions = ()) returns ResourceResponse|error {
